...
# Function to display metrics later for streamlit
...
# Streamlit app coding

st.title("Multiple Disease Prediction App")

page = st.sidebar.selectbox("Select Page", ["Prediction", "Model Metrics"])
    ...
    if disease == "Parkinson's Disease":
        ...
        for col in X_kidney.columns:
            if col in ['rbc', 'pc']:
                ...
                input_data[col] = st.selectbox(col, options=['present', 'not present'])
